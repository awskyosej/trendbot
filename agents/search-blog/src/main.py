"""
AWS 블로그 검색 에이전트 - AgentCore Runtime + BrowserClient
"""
...
async def search_web(query: str) -> str:
    ...
    try:
        ...
        task = (
            ...
            f"AWS 블로그 게시물을 찾아 제목, URL, 날짜, 카테고리, 요약을 JSON 배열로 반환하세요. "
            f"단순 출시 공지와 docs.aws.amazon.com은 제외하세요."
        )
...
async def invoke(payload: Dict[str, Any], context=None):
    ...
    prompt = payload.get("prompt") or f"AWS 블로그에서 {period} 기간의 아키텍처/구현 모범사례를 검색해 주세요. site:aws.amazon.com/blogs"

    model = BedrockModel(model_id=AGENT_MODEL)
    agent = Agent(
        model=model,
        tools=[search_web],
        system_prompt="AWS 블로그 검색 전문가입니다. search_web 도구로 검색하고 모범사례만 JSON으로 반환하세요.",
    )
