"""
AWS 블로그 검색 에이전트 - AgentCore Runtime

AgentCore 브라우저 도구로 AWS 블로그(한국어+글로벌)에서
아키텍처/구현 모범사례를 검색합니다.
"""
...
SYSTEM_PROMPT = """당신은 AWS 블로그 검색 전문가입니다.
run_browser_search 도구를 사용하여 AWS 블로그에서 모범사례를 검색하세요.
단순 출시 공지와 AWS Documentation은 제외하세요.
결과를 JSON 형식으로 반환하세요. 반드시 실제 URL을 포함하세요."""


@tool
async def run_browser_search(query: str, starting_page: str = "https://www.google.com/") -> str:
    """웹 브라우저로 AWS 블로그를 검색합니다.

    query: 검색어 (예: "AWS architecture best practices blog")
    ...
    try:
        ...
        task = (
            ...
            f"검색 결과에서 AWS 블로그 게시물을 찾아 다음 정보를 추출하세요:\n"
            f"- 블로그 제목\n"
            f"- 블로그 URL (실제 링크)\n"
            f"- 게시일\n"
            f"- 카테고리 (아키텍처/구현사례/보안/비용최적화 등)\n"
            f"- 블로그 요약 (100자 이내)\n"
            f"단순 출시 공지와 docs.aws.amazon.com 페이지는 제외하세요.\n"
            f"JSON 배열로 반환하세요."
        )
...
async def invoke(payload: Dict[str, Any], context=None) -> Dict[str, Any]:
    ...
    if not message:
        message = f"AWS 블로그에서 {search_period} 기간의 아키텍처/구현 모범사례를 검색해 주세요. site:aws.amazon.com/blogs"
