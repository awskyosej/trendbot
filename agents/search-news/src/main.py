"""
뉴스 검색 에이전트 - AgentCore Runtime + BrowserClient
"""
...
async def search_web(query: str) -> str:
    ...
    try:
        ...
        task = (
            ...
            f"검색 결과에서 뉴스 기사 5개를 찾아 제목, URL, 출처, 날짜, 요약을 JSON 배열로 반환하세요."
        )
...
async def invoke(payload: Dict[str, Any], context=None):
    ...
    prompt = payload.get("prompt") or f"{customer} 관련 최신 뉴스를 {period} 기간으로 검색해 주세요."

    model = BedrockModel(model_id=AGENT_MODEL)
    agent = Agent(
        model=model,
        tools=[search_web],
        system_prompt="뉴스 검색 전문가입니다. search_web 도구로 검색하고 결과를 JSON으로 반환하세요.",
    )
