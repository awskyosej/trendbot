"""
뉴스 검색 에이전트 - AgentCore Runtime

AgentCore 브라우저 도구(BrowserClient)를 사용하여
고객사 관련 뉴스를 실제 웹 검색합니다.
"""
...
logger = logging.getLogger("search-news")

SYSTEM_PROMPT = """당신은 뉴스 검색 전문가입니다.
사용자가 요청한 고객사의 최신 뉴스를 run_browser_search 도구를 사용하여 검색하세요.
검색 결과를 JSON 형식으로 정리하여 반환하세요.
반드시 실제 URL을 포함하세요."""
...
async def run_browser_search(query: str, starting_page: str = "https://www.google.com/") -> str:
    ...
    try:
        ...
        task = (
            ...
            f"검색 결과에서 뉴스 기사를 찾아 다음 정보를 추출하세요:\n"
            f"- 기사 제목\n"
            f"- 기사 URL (실제 링크)\n"
            f"- 출처\n"
            f"- 게시일\n"
...
async def invoke(payload: Dict[str, Any], context=None) -> Dict[str, Any]:
    ...
    if not message and customer_name:
        message = f"{customer_name} 관련 최신 뉴스를 {search_period} 기간으로 검색해 주세요."
